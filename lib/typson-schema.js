...
(function (underscore, Q, _traverse, typson) {
    ...
    api.schema = function (script, type, id) {
        return Q.promise(function (resolve) {
            api.definitions(script, "#/definitions").done(function(definitions) {
                var schema = {};
                 // schema.$schema = "http://json-schema.org/draft-04/schema#"; // not necessary
                _.extend(schema, definitions[type]);
                delete definitions[type];
                // if(id) {
                //     schema.id = id; // not necessary
                // }
                resolve(schema);
    ...
    function handleInterfaceDeclaration(type, definitions, refPath, modulePath) {
        ...
        // definition.id = name; // this property renders the definitions invalid for Swagger
        // definition.type = "object"; // not necessary
    ...
    function handleEnumDeclaration(type, definitions, modulePath) {
        ...
        // definition.id = type.name.actualText; // this property renders the definitions invalid for Swagger
